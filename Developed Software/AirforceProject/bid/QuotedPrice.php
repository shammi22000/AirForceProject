...
//====================================================================================
 			...
 			while ($newArray111 = mysql_fetch_array($result111))
			{
				...
				include("../Include_Connection.php");	  
				$sql21="SELECT quanItemID, actualQuantity FROM quantity WHERE quanItemID='$priceItemID' and projectYear='$_GET[puryear]' and quanstaID='$_GET[PurStation]'";
				$result21 = mysql_query($sql21, $conn); 
				$actualQuantityTot=0;
				while($newArray21 = mysql_fetch_array($result21)) {
				$actualQuantityTot=$actualQuantityTot+$newArray21['actualQuantity']; }
//=========================================================================================
			...
			$sql112="INSERT INTO bidquote (bidQuoteID,  date, projYear,  bQitemID, bQstaID, bQcatID, bQSupId, priceQuote, subTot) VALUES ('', '$date', '$proYear', '$priceItemID', '$pricestaID', '$CatId', '$bidder', '$price', '$grand')";
...
function validate123(){
	...
	for(var i=0; i< elems.length; i++){
		if(elems[i].type=='text' && parseInt(elems[i].value) != elems[i].value){
...
<form name="form1"  method="get" action="<?php $_SERVER['PHP_SELF']?>" >
 ...
 <hr/>      
      			<?php //retrieve selected stations for bid infor ========================================
  	 			error_reporting(E_ALL ^ E_NOTICE);
				include ("../include_Connection.php");
...
<p align="left">Station Name
				...
				<?php  }  ?> 
        		</select>&nbsp;&nbsp; <input type="submit" name="serStat" value="SELECT" />
</p>
...
<!-- /////////////////////////////form2 starting////////////////////////////// --> 
<form name="price_Form"  method="get" action="<?php $_SERVER['PHP_SELF']?>" id="price_Form" onsubmit="javascript: return validate123();"  >
  ...
  <tr>
        <td colspan="2" align="center">
        	...
        	<input type="hidden" name="puryear" value="<?php echo $projectYear; ?>" /> 
         	<input name="actQuan" type="hidden" value="<?php echo $actualQuantityTot; ?>" />
        </td>
      			<?php }?>	
</tr>
</table>
