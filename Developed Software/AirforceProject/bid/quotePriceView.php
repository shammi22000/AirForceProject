...
<table width="990" border="1" cellspacing="0" cellpadding="0" class="maintable" bgcolor="#A8CBF7">
 ...
 <tr>
  ...
  <tr>
    ...
    <td width="790" height="435" valign="top" align="center" bgcolor="#FFFFFF"><h2><br/>Quoted Price List View Page</h2><br/>
...
<hr/>
      			...
      			<?php //retrieve bid quoted stations ========================================================
  	 				error_reporting(E_ALL ^ E_NOTICE);
					include ("../include_Connection.php");
...
<p align="left">Station Name :  
				...
				?>
     			</select>&nbsp;&nbsp; <input type="submit" name="seleStat" value="SELECT" /> 
</p>
       
     
     			<?php //retrieve bid quoted category ========================================================
 					if(isset($_GET['seleStat']))
					{
	 				$Id= $_GET['SerStat'];
		 			include("../Include_Connection.php");
 					$sql ="SELECT * FROM  category,  bidquote WHERE (category.catId=bidquote.bQcatID AND bQSupId ='$bidID' AND bQstaID='$Id' AND projYear='$projectYear' ) GROUP BY catName ";
 	 				$result = mysql_query($sql, $conn) or die(mysql_error()); 
 				?>
...
<form name="form2"  method="get" action="<?php $_SERVER['PHP_SELF']?>">
       			<?php
					...
					$statID=$_GET['SerStat'];
				 	include("../Include_Connection.php");
					$sql1 ="SELECT * FROM item WHERE (itemCatID='$Id')";
...
<tr class="<?php echo $bg; ?>">
    	...
    	<td><?php echo $newArray1['weight']; ?><?php echo $newArray1['UOM']; ?></td>
	
    
				<?php
					$itemID=$newArray1['itemID']; 
					include("../Include_Connection.php");  
					$sql2="SELECT quanItemID, actualQuantity FROM quantity WHERE quanItemID='$itemID' and projectYear='$projectYear' and quanstaID='$_GET[SerStat]'";
        ...
        <?php
					...
					include("../Include_Connection.php");
					$sql5 ="SELECT * FROM bidquote  WHERE (projYear='$projectYear' AND bQcatID='$Id' AND bQstaID='$statID' AND bQitemID='$itemID' AND bQSupId='$bidID')";
					//execute the SQL statement***************
	 				$result5 = mysql_query($sql5, $conn);
					while ($newArray5 = mysql_fetch_array($result5))
 				{
 		 		?>
    	<td align="right"> <?php  $quotePrice=$newArray5['priceQuote']; echo $newArray5['priceQuote']; }?></td>
         <?php
 				...
 				{
					?>
    	<td align="right"> <?php $subTot=$newArray5['subTot']; echo $newArray5['subTot'];  }?></td>
				...
				<?php
					...
					//execute the SQL statement***************
	 				...
	 				$result5 = mysql_query($sql5, $conn);?>
					<?php while ($newArray5 = mysql_fetch_array($result5))
 				{
    	...
    	<td align="center"> <a href="quotePriceEdit.php?ID=<?php echo $newArray1['bidQuoteID']; ?>"><img src="../images/b_edit.png" width="16" height="16" alt="EDIT" border="0" /></a>&nbsp;&nbsp; </td>   
 </tr>
				<?PHP  $a++; } ?><?PHP ?>
<tr>
		<td>&nbsp;</td>
		<td colspan="4" align="right"><strong>Grand Total (Rs)</strong></td>
		<th align="right"><?php echo $EQTotal; ?></th>
  		<td>&nbsp;</td>
